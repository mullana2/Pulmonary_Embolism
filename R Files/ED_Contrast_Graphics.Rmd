...
Scanner <- PE %>% group_by(CT.Type) %>% summarise(n = n(), Radiation = mean(Contrast.Given),
                                                    Contrast = mean(Contrast.Given))

bar_NumScanner <- ggplot(Scanner, aes(fill = CT.Type, x = CT.Type,
                                       y = n, label = n)) +
                     geom_bar(position = "dodge", stat = "identity", width = 0.5) +
                     geom_text(size = 3, position = position_dodge(0.5), vjust = -1) +
                     scale_fill_manual(values = c("lightblue", "dodgerblue" ,"mediumblue" ,"navyblue")) +
...
box_ScannerED <- ggplot(PE, aes(x = CT.Type, y = Contrast.Given, fill = CT.Type)) +
                geom_boxplot() +
                scale_fill_manual(values = c("lightblue", "dodgerblue" ,"mediumblue" ,"navyblue")) +
...
box_Gender <- ggplot(PE, aes(x = Gender, y = Contrast.Given, fill = Gender)) +
                geom_boxplot() +
...
box_Age <- ggplot(PE, aes(x = Age_range, y = Contrast.Given, fill = Age_range)) +
                geom_boxplot() +
...
box_BMI <- ggplot(PE, aes(x = BMI_range, y = Contrast.Given, fill = BMI_range)) +
                geom_boxplot() +
                scale_fill_manual(values = c("lightblue","dodgerblue", "mediumblue")) +
                ylab("Effective Dose (mSv)") + 
                xlab("Patient Gender") +
...
box_Location <- ggplot(PE, aes(x = Location, y = Contrast.Given, fill = Location)) +
                geom_boxplot() +
                scale_fill_manual(values = c("lightblue","dodgerblue", "mediumblue", "navyblue")) +
                ylab("Contrast (mL)") + 
                xlab("Patient Location") +
...
ggsave(filename = "EffDose_Contrast_Manuscript/Images/Contrast/Box_Location.jpeg",
       plot = box_Location, width = 7, height = 5, device = "jpeg")
